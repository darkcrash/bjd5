...
namespace Bjd.DhcpServer
{
    ...
    public class Lease : IDisposable
    {
        ...
        void Save()
        {
            ...
            using (StreamWriter sw = new StreamWriter(bs, Encoding.ASCII))
            {
                for (int i = 0; i < ar.Count; i++)
                {
        ...
        void Read()
        {
            if (!File.Exists(fileName))
                return;
            using (FileStream bs = new FileStream(fileName, FileMode.Open))
            using (StreamReader sr = new StreamReader(bs, Encoding.ASCII))
            {
                while (true)
                {
