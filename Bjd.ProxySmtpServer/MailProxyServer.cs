...
namespace Bjd.ProxySmtpServer
{
    ...
    abstract class MailProxyServer : OneServer
    {
        ...
        abstract protected string BeforeJob(ISocket client, List<byte[]> clientBuf);//接続前の処理
        abstract protected string ConnectJob(ISocket client, ISocket server, List<byte[]> clientBuf);//接続後の処理
