...
namespace Bjd.WebApiServer
{
    class SvMail
    {
        ...
        public SvMail(Kernel kernel)
        {
            ...
            _mailQueue = Define.ExecutableDirectory + $"{Path.DirectorySeparatorChar}MailQueue";
        }
            ...
            else if (method == Method.Delete)
            {
                ...
                foreach (var o in GetMailList(owner, limit))
                {
                    if (o.Owner == "mailQueue")
                    {
                        ...
                        DeleteFile(_mailBox.Dir + Path.DirectorySeparatorChar + o.Owner, (string)o.Get("filename"));
                        count++;
                    }
                }
        ...
        void DeleteFile(String dir, String filename)
        {
            ...
            var path = $"{dir}{Path.DirectorySeparatorChar}MF_{filename}";
            if (File.Exists(path))
            {
                File.Delete(path);
            }
            //path = string.Format("{0}\\DF_{1}", dir, filename);
            path = $"{dir}{Path.DirectorySeparatorChar}DF_{filename}";
            if (File.Exists(path))
            {
                File.Delete(path);
            }
            ...
            if (_mailBox != null)
            {
                foreach (var user in _mailBox.UserList)
                {
                    if (owner.Count == 0 || owner.IndexOf(user) != -1)
                    {
                        ...
                        var folder = $"{_mailBox.Dir}{Path.DirectorySeparatorChar}{user}";
