...
namespace Bjd.ProxyHttpServer
{
    class OneProxyHttp : IDisposable
    {
        ...
        {
            //キャッシュ対象外
            if (!_isCacheTarget) return;

            //レスポンスコードが200のものだけが対象
            if (_response.Code != 200) return;

            //Ver5.6.1
            if (!_oneObj.Body[CS.Server].CanUse) return;
        ...
        public async ValueTask<bool> SendServerAsync(ILife iLife)
        {
            //処置なし
            if (_sideState[CS.Server] != HttpSideState.Non && _sideState[CS.Server] != HttpSideState.ServerSideSendHeader)
                return true;
            ...
            if (_sideState[CS.Server] == HttpSideState.Non)
            {
                ...
                //送信
                if (!await SendAsync(_proxy.Sock(CS.Server), sendBuf)) return false;
                _sideState[CS.Server] = HttpSideState.ServerSideSendHeader;
            }
            ...
            {
                //バッファに残っているデータの送信
                if (!await SendBufAsync(CS.Client)) return false;
            }
        ...
        {
            //処置なし
            if (_sideState[CS.Server] == HttpSideState.ServerSideRecvBody) return true;
            ...
            if (_sideState[CS.Server] == HttpSideState.ServerSideSendBody)
            {
                ...
                //ヘッダの受信
                if (!await _oneObj.Header[CS.Server].RecvAsync(_proxy.Sock(CS.Server), _proxy.OptionTimeout, iLife))
                {
                    _proxy.Logger.Set(LogKind.Error, _proxy.Sock(CS.Server), 7, "");
                    return false;
                }
            ...
            { //チャンク形式の場合
                //チャンク形式の受信
                if (!await RecvServerChunkAsync(iLife))
                    return false;
            }
            else
            { //Content-Length形式の受信
                if (!await RecvServerContentLengthAsync(iLife))
                    return false;
            }
        ...
        async ValueTask<bool> RecvServerChunkAsync(ILife iLife)
        {
            var len = _proxy.Sock(CS.Server).Length();
            if (len <= 0)
                return true;
            while (iLife.IsLife())
            {
                ...
                { //チャンクサイズの取得
                    ...
                    using (var b1 = await _proxy.Sock(CS.Server).LineBufferRecvAsync(_proxy.OptionTimeout))
                    {
                        ...
                        _oneObj.Body[CS.Server].Add(b1);
                        _lastRecvServer = DateTime.Now.Ticks;

                        //サイズ変換
                        string str = Encoding.ASCII.GetString(b1.Data, 0, b1.DataSize - 2);
                        if (str == "")
                        {
                            _chunkLen = -1;//次回はサイズ取得
                            continue;
                        ...
                        { //受信完了
                          //残りのデータ(空行)ある場合
                            ...
                            if (0 < l)
                            {
                                using (var b2 = await _proxy.Sock(CS.Server).BufferRecvAsync(l, _proxy.OptionTimeout))
                                {
                                    if (b2 != null) _oneObj.Body[CS.Server].Add(b2);
                                }
                            }
                            SetServerSideBody(); //Ver5.7.2
                                                 //_sideState[CS.Server] = HttpSideState.ServerSideRecvBody;
                ...
                //データ受信（サイズ分）
                using (var b3 = await _proxy.Sock(CS.Server).BufferRecvAsync(_chunkLen, _proxy.OptionTimeout))
                {
                    if (b3 == null || b3.DataSize != _chunkLen)
                        return false;

                    _oneObj.Body[CS.Server].Add(b3);

                    if (_isText)
                    {
                        _textData = Bytes.Create(_textData, b3.Data);
                        //コンテンツ制限の確認
                        if (IsHitLimitString(_textData))
                            return false;
                    }
        ...
        public bool SendClient(ILife iLife)
        {
            ...
            else if (_oneHttpKind == OneHttpKind.Chunk)
            { //chunkの場合
        ...
        public async ValueTask<bool> SendClientAsync(ILife iLife)
        {
            if (_sideState[CS.Server] != HttpSideState.ServerSideRecvHeader && _sideState[CS.Server] != HttpSideState.ServerSideRecvBody)
                return true;
            ...
            if (_sideState[CS.Client] == HttpSideState.ClientSideRecvRequest)
            {
                //サーバから受信したレスポンス及びヘッダをバッファに展開する
                var sendBuf = Bytes.Create(_response.ToString(), "\r\n", _oneObj.Header[CS.Server].GetBytes());
                if (!await SendAsync(_proxy.Sock(CS.Client), sendBuf))//送信
                    return false;
                _sideState[CS.Client] = HttpSideState.ClientSideSendHeader;//ヘッダ送信完了
            }
            if (_sideState[CS.Client] == HttpSideState.ClientSideSendHeader)
            {
                //バッファに残っているデータの送信
                if (!await SendBufAsync(CS.Server))
                    return false;
            }
