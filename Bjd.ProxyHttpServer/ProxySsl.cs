...
namespace Bjd.ProxyHttpServer
{
    ...
    class ProxySsl : ProxyObj
    {
        ...
        public override async ValueTask<bool> PipeAsync(ILife iLife)
        {
            ...
            if (!await RecvClientAsync(iLife))//クライアントからの受信
                return false;
            if (!await SendServerAsync(iLife))//サーバへの送信
                return false;
            if (!await RecvServerAsync(iLife))//サーバからの受信
                return false;
            if (!await SendClientAsync())//クライアントへの送信
                return false;
        ...
        async ValueTask<bool> RecvClientAsync(ILife iLife)
        {
            if (!await RecvBufAsync(CS.Client, iLife))
                return false;
            return true;
        }
        ...
        {
            if (!await RecvBufAsync(CS.Server, iLife))
                return false;
            return true;
        }
        ...
        async ValueTask<bool> SendServerAsync(ILife iLife)
        {
            ...
            //バッファに残っているデータの送信
            if (!await SendBufAsync(CS.Client))
                return false;
            return true;
        }
        ...
        async ValueTask<bool> RecvBufAsync(CS cs, ILife iLife)
        {
            ...
            if (len == 0)
                return true;
            using (var b = await sock.BufferRecvAsync(len, Proxy.OptionTimeout))
            {
                if (b == null)
                    return false;
                _oneObj.Body[cs].Add(b);
