...
namespace Bjd.ProxyHttpServer
{
    class OneObj : IDisposable
    {
        ...
        public bool RecvRequest(bool useRequestLog, LimitUrl limitUrl, ILife iLife)
        {
            ...
            //if (Request.Protocol == ProxyProtocol.Http && Request.HttpMethod == HttpMethod.Post)
            //{//POSTの場合
        ...
        public async ValueTask<bool> RecvRequestAsync(bool useRequestLog, LimitUrl limitUrl, ILife iLife)
        {

            //リクエスト取得（内部データは初期化される）ここのタイムアウト値は、大きすぎるとブラウザの切断を取得できないでブロックしてしまう
            if (!await Request.RecvAsync(Proxy.Logger, Proxy.Sock(CS.Client),/*timeout 3 */ Proxy.OptionTimeout, iLife))
            {
                return false;
            }

            //ヘッダの取得
            if (!await Header[CS.Client].RecvAsync(Proxy.Sock(CS.Client), Proxy.OptionTimeout, iLife))
            {
                return false;
            }
            ...
            if (Request.Protocol == ProxyProtocol.Http)
            {
                    ...
                    if (strContentLength != null)
                    {
                        try
                        {
                            ...
                            if (0 < len)
                            {
                                ...
                                {
                                    var size = len - buf.Length;
                                    using (var b = await Proxy.Sock(CS.Client).BufferRecvAsync(size, Proxy.OptionTimeout))
                                    {
                                        buf = Bytes.Create(buf, b.Data);
                                    }
                                    if (len <= buf.Length)
                                    {
                                        break;
                ...
                {
                    //using (var b = await Proxy.Sock(CS.Client).LineBufferRecvAsync(Proxy.OptionTimeout))
                    //{
                    //}
                }
