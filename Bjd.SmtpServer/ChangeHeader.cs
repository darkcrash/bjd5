...
namespace Bjd.SmtpServer
{
    class ChangeHeader
    {
        ...
        public ChangeHeader(IEnumerable<DatRecord> replace, IEnumerable<DatRecord> append)
        {
            if (replace != null)
            {
                foreach (var d in replace)
                {
                    if (d.Enable)
                    {
                        _replace.Add(d.ColumnValueList[0], d.ColumnValueList[1]);
                    }
                }
            }
            if (append != null)
            {
                foreach (var d in append)
                {
                    if (d.Enable)
                    {
                        _append.Add(d.ColumnValueList[0], d.ColumnValueList[1]);
                    }
                }
            }
