...
namespace Bjd.FtpServer
{
    ...
    public class CurrentDir
    {
        ...
        public CurrentDir(string homeDir, ListMount listMount)
        {
            ...
            {
                _homeDir = homeDir + Path.DirectorySeparatorChar;
            }
            else {
                _homeDir = homeDir;
            }
            ...
            if (_oneMount == null)
            {
                ...
                foreach (var a in _listMount)
                {
                    if (a.IsToFolder(_current))
                    {
                        ...
                        if ($"{_current}{a.Name}{Path.DirectorySeparatorChar}" == path)
                        {
                            //_current = a.FromFolder + "\\";
                            _current = a.FromFolder + Path.DirectorySeparatorChar;
                            _oneMount = a;
                            return true;
                        }
                    }
                ...
                path = $"{_oneMount.ToFolder}{Path.DirectorySeparatorChar}{_oneMount.Name}{s}";
        ...
        public string GetPwd()
        {
            ...
            tmpStr = tmpStr.Replace(Path.DirectorySeparatorChar, '/');
        ...
        public string CreatePath(string path, string param, bool isDir)
        {
            ...
            //param = param.Replace('/', '\\');
            param = param.Replace('/', Path.DirectorySeparatorChar);

            //paramの'\\'が連続している個所を１つにまとめる
            //param = MargeChar('\\', param);
            param = MargeChar(Path.DirectorySeparatorChar, param);
            ...
            if (isDir)
            {
                ...
                //    param = param + "\\";
                //}
                if (param[param.Length - 1] != Path.DirectorySeparatorChar)
                {
                    param = param + Path.DirectorySeparatorChar;
                }
            ...
            // フルパスで指定されている場合
            //if (param[0] == '\\')
            if (param[0] == Path.DirectorySeparatorChar)
            {
            ...
            dir = Path.GetDirectoryName(dir) + Path.DirectorySeparatorChar;
            ...
            while (true)
            {
                ...
                var p2 = tmpStr.LastIndexOf(Path.DirectorySeparatorChar);
                if (p2 < 0)
                {
                    return null;
                }
            ...
            newPath = MargeChar(Path.DirectorySeparatorChar, newPath);
            //\\.\\は\\にまとめる
