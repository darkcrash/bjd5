...
namespace Bjd
{
    public class Lookup
    {
        ...
        static public List<string> DnsServer()
        {
            ...
            {
                if (!ni.Supports(System.Net.NetworkInformation.NetworkInterfaceComponent.IPv4)) continue;
                foreach (var dnsAddr in ni.GetIPProperties().DnsAddresses)
                {
                    if (dnsAddr.IsIPv4MappedToIPv6) continue;
                    if (dnsAddr.IsIPv6LinkLocal) continue;
                    if (dnsAddr.IsIPv6Multicast) continue;
                    if (dnsAddr.IsIPv6SiteLocal) continue;
                    if (dnsAddr.IsIPv6Teredo) continue;
                    list.Add(dnsAddr.ToString());
                }
            }
        ...
        static public List<string> QueryA(string hostName)
        {
            ...
            try
            {
                var hostEntryAwait = Dns.GetHostEntryAsync(hostName);
                hostEntryAwait.Wait();
                var hostEntry = hostEntryAwait.Result;
